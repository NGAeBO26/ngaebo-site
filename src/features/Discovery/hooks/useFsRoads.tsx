...
function getRoadsBeforeLayerId(map: MaplibreMap): string | undefined {
  try {
    ...
    const labelLayer = layers.find((l: any) => /label|symbol/i.test(l.id));
...
export default function useFsRoads(
  ...
  useEffect(() => {
    ...
    try {
      if (opts?.activeHoverId && routesData.length > 0) {
        if (
          opts?.isTakeoverActive &&
          opts?.activeHoverId === opts?.selectedRouteId
        ) {
        ...
        const match = routesData.find(
          (r: any) =>
            r &&
            String(r.properties?.profile_id || r.id) ===
              String(opts.activeHoverId),
        );
        if (match?.properties) {
    ...
    map,
    mapReady,
    options?.activeHoverId,
    options?.selectedRouteId,
    routesData,
    options?.isTakeoverActive,
  ]);
  ...
  useEffect(() => {
    ...
    const ensureSourceAndMaybeLayers = async (
      ...
      try {
        const res = await fetch("/data/rideguide_routes_v3.geojson", {
          cache: "no-store",
        });
