...
export default function TokenUpsellModal({
  ...
  const [isMobileLeadExpanded, setIsMobileLeadExpanded] = useState(false);
  ...
  const handleUpgradeAction = async (tier: UpsellTier) => {
    ...
    try {
      /* 🎯 STATE PERSISTENCE ARMOR: Backup state tracking cache immediately before async batch execution threads begin */
      localStorage.setItem("rg_active_token_pack", JSON.stringify({
        id: "TOKEN_BUNDLE_BACKUP_NODE",
        routeId: "TOKEN_BUNDLE",
        title: tier.title,
        price: tier.price,
        distance: tier.badgeText || "Credit Bundle Package",
        fcsLabel: "premium",
        isTokenBundle: true
      }));
      ...
      /* 🎯 RECOVERY DISPATCH: Wipe local storage backup if Shopify API fails */
  ...
  return (
    <div className="rg-upsell-backdrop" onClick={onClose} role="presentation">
      <div className="rg-upsell-container" role="dialog" aria-modal="true" aria-labelledby="rg-upsell-title" aria-describedby="rg-upsell-description" tabIndex={-1} ref={modalRef} onClick={(e) => e.stopPropagation()}>
        ...
        {/* 🎯 ACORDION WRAPPER TRAY: Listens to our expansion hook matrix seamlessly */}
        <section 
          className={`lead-capture-footer mod-upsell-override ${isMobileLeadExpanded ? "mod-expanded" : "mod-collapsed"}`} 
          aria-label="Free Map Sample Giveaway Registration"
        >
          <div className="funnel-container">
            <div className="capture-split-layout">
              
              {/* 🎯 CLICKABLE TRIGGER PANELS: Becomes a hot tap hitbox row on mobile devices */}
              <div 
                className="capture-text-stack"
                onClick={() => setIsMobileLeadExpanded(!isMobileLeadExpanded)}
                role="button"
                aria-expanded={isMobileLeadExpanded}
                tabIndex={0}
              >
                <h3 className="lead-footer-headline">
                  Get Your Free Sample Pack 
                  <span className="rg-accordion-indicator-arrow">
                    {isMobileLeadExpanded ? " ▼" : " ▲"}
                  </span>
                </h3>
